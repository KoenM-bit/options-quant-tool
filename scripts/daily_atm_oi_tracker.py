#!/usr/bin/env python3
"""
Daily ATM Options Analysis - OI and Volume Changes

Analyzes daily OI and volume changes for ATM and surrounding strikes (±6 strikes)
for the current expiry month and next expiry month.

Shows day-over-day changes in Open Interest and Volume.

Usage:
    python scripts/daily_atm_oi_tracker.py
    python scripts/daily_atm_oi_tracker.py --ticker AH
    python scripts/daily_atm_oi_tracker.py --strike-range 8  # ±8 strikes instead of ±6
    python scripts/daily_atm_oi_tracker.py --days 10         # Look back 10 days
"""
...
def get_daily_oi_volume_data(engine, ticker=None, strike_range=6, lookback_days=10):
    """
    Get OI and volume data for recent days, focusing on ATM and nearby strikes
    ...
    WITH recent_data AS (
        ...
        WHERE scraped_at::date >= CURRENT_DATE - interval '{lookback_days} days'
            {ticker_filter}
    ...
    -- Get latest underlying price (or estimate from strike distribution)
    latest_price AS (
        SELECT DISTINCT ON (ticker)
            ticker,
            COALESCE(
                underlying_last_price,
                -- If no underlying price, estimate from middle of strike range
                (SELECT (MIN(strike) + MAX(strike)) / 2 
                 FROM recent_data rd2 
                 WHERE rd2.ticker = recent_data.ticker 
                   AND rd2.open_interest > 0)
            ) as underlying_last_price
        FROM recent_data
    ...
    SELECT 
        ...
        rd.strike,
        rd.expiration_date,
        rd.actual_expiration_date,
        rd.open_interest,
        COALESCE(rd.volume, 0) as volume,
        rd.trade_date,
        lp.underlying_last_price,
        rd.strike - lp.underlying_last_price as distance_from_atm,
        ABS(rd.strike - lp.underlying_last_price) as abs_distance
    FROM recent_data rd
    INNER JOIN next_expiries ne 
        ON rd.exp_year = ne.exp_year 
        AND rd.exp_month = ne.exp_month
    LEFT JOIN latest_price lp 
        ON rd.ticker = lp.ticker
...
def calculate_daily_changes(df):
    """Calculate day-over-day changes in OI and volume."""
    if df.empty:
        return df
...
def print_daily_comparison_table(df, expiry_label):
    """Print a formatted table showing daily OI and volume changes."""
    if df.empty:
    ...
    # Get unique dates (show most recent transitions)
    dates = sorted(df['to_date'].unique(), reverse=True)[:5]  # Show last 5 days of changes
    ...
    for opt_type in ['C', 'P']:
        ...
        for strike in strikes:
            strike_df = type_df[type_df['strike'] == strike].sort_values('to_date', ascending=False)
            ...
            moneyness = "ATM" if abs(distance) < 0.5 else ("ITM" if (opt_type == 'C' and distance < 0) or (opt_type == 'P' and distance > 0) else "OTM")
            
            print(f"\n  Strike: €{strike:.2f} ({moneyness}, {distance:+.2f} from €{underlying:.2f})")
            
            # Print each day's data (most recent first)
            for _, row in strike_df.head(5).iterrows():
...
def print_summary_stats(df, expiry_label):
    ...
    print(f"  Last Day Vol - Calls: {total_vol_calls:,.0f} | Puts: {total_vol_puts:,.0f}")
...
def main():
    parser = argparse.ArgumentParser(description="Daily ATM Options OI & Volume Analysis")
    parser.add_argument("--ticker", type=str, default=None, help="Filter by ticker (e.g., AH)")
    ...
    # Print report header
    print("\n" + "█" * 100)
    print("  📊 DAILY ATM OPTIONS ANALYSIS - OI & VOLUME CHANGES")
    print("█" * 100)
    ...
    # Get data
    print("\n⏳ Fetching daily data from database...")
    df = get_daily_oi_volume_data(engine, args.ticker, args.strike_range, args.days)
    
    if df.empty:
        print("\n❌ No data found for the specified criteria.")
        print("   Make sure you have data in bronze_euronext_options table.")
        return
    ...
    # Calculate changes
    changes_df = calculate_daily_changes(df)
    
    if changes_df.empty:
        print("\n❌ Not enough data to calculate daily changes.")
    ...
    print_daily_comparison_table(current_expiry_df, f"Expiry: {current_expiry_date}")
    ...
    if len(expiries) > 1:
        ...
        print_daily_comparison_table(next_expiry_df, f"Expiry: {next_expiry_date}")
    ...
    print(f"\n📈 Top OI Increases (Last Day: {latest_date.strftime('%Y-%m-%d')}):")
    ...
    print(f"\n📉 Top OI Decreases (Last Day: {latest_date.strftime('%Y-%m-%d')}):")
